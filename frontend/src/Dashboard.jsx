// frontend/src/Dashboard.jsx - Complete Version with Manual Ping Testing
...
const Dashboard = ({ token, onLogout }) => {
  ...
  // Ping testing state - NEW
  const [pingingDevices, setPingingDevices] = useState(new Set())
  const [pingResults, setPingResults] = useState({})
  
  const [newDevice, setNewDevice] = useState({
    name: '', device_type: 'server', operating_system: 'ubuntu',
    os_version: '', ip_address: '', hostname: '',
    business_criticality: 'medium', environment: 'production',
    description: '', location: '', owner_contact: ''
  })
  ...
  const operatingSystems = [
    ...
    { value: 'windows-server-2016', label: '🪟 Windows Server 2016', category: 'Windows' },
      ...
      wsRef.current.onmessage = (event) => {
        ...
        if (data.type === 'device_health_update') {
          ...
          if (data.data.user !== userProfile.username) {
            showMessage(`🏓 ${data.data.user} pinged ${data.data.device_name}: ${data.data.status.toUpperCase()}`)
          }
  ...
  useEffect(() => {
    ...
    if (filterEnvironment) {
      filtered = filtered.filter(device => device.environment === filterEnvironment)
    }
  ...
  // NEW - Manual ping test function
  const pingDevice = async (deviceId, deviceName, deviceIp) => {
    if (!deviceIp) {
      showMessage('❌ No IP address to ping')
      return
    }
    ...
    try {
      const response = await axios.post(`${API_URL}/api/devices/${deviceId}/ping?token=${token}`)
      ...
      const statusIcon = response.data.success ? '🟢' : '🔴'
      const responseTime = response.data.response_time > 0 ? ` (${response.data.response_time.toFixed(1)}ms)` : ''
      showMessage(`🏓 ${statusIcon} ${deviceName}: ${response.data.status.toUpperCase()}${responseTime}`)
      ...
      showMessage(`❌ Failed to ping ${deviceName}`)
    } finally {
  ...
  const updateDevice = async (e) => {
    ...
    try {
      await axios.put(`${API_URL}/api/devices/${selectedDevice.id}?token=${token}`, selectedDevice)
  ...
  const changePassword = async (e) => {
    ...
    try {
      await axios.post(`${API_URL}/api/change-password?token=${token}`, passwordChange)
  ...
  const showMessage = (msg) => {
    setMessage(msg)
    setTimeout(() => setMessage(''), 4000) // Updated to 4 seconds for ping results
  }
    ...
    <div style={{
          ...
          {[
            ...
            <div
              ...
              style={{
                ...
                padding: '12px 16px', margin: '4px 0', borderRadius: '8px',
                background: activeTab === item.id ? '#f0f4ff' : 'transparent',
                color: activeTab === item.id ? '#6366f1' : '#64748b',
                cursor: 'pointer', transition: 'all 0.2s ease',
                fontSize: '15px', fontWeight: activeTab === item.id ? '600' : '500'
              }}
            >
      ...
      <div style={{ flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ margin: '4px 0 0 0', color: '#64748b', fontSize: '16px', display: 'flex', gap: '16px' }}>
              <span>{dashboardData.total_devices || 0} devices registered</span>
              {dashboardData.online_devices > 0 && (
                <span style={{ color: '#10b981' }}>
                  {dashboardData.online_devices} online
                </span>
              )}
              {dashboardData.avg_response_time > 0 && (
                <span style={{ color: '#6366f1' }}>
                  Avg ping: {dashboardData.avg_response_time}ms
                </span>
              )}
              <span>Manual ping testing</span>
            </div>
          </div>
          <button
        ...
        {/* Content Area */}
        <div style={{ padding: '32px', height: 'calc(100vh - 120px)', overflow: 'auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', height: '100%' }}>
              ...
              <div style={{
                ...
                <div style={{ textAlign: 'center', padding: '20px 0' }}>
                  <div style={{
                    fontSize: '48px', fontWeight: '700', color: '#6366f1',
                    margin: '0 0 8px 0'
                  }}>
                    {devices.filter(d => d.status !== 'unknown').length > 0 
                      ? Math.round((devices.filter(d => d.status === 'online').length / devices.filter(d => d.status !== 'unknown').length) * 100)
                      : 'N/A'
                    }
                  </div>
                  <div style={{ color: '#64748b', fontSize: '14px', marginBottom: '16px' }}>
                    ...
                    {dashboardData.avg_response_time > 0 && (
                      <div style={{ fontSize: '12px', marginTop: '4px' }}>
                        Average Response: {dashboardData.avg_response_time}ms
                      </div>
                    )}
                    <div style={{ fontSize: '12px', marginTop: '4px', fontStyle: 'italic' }}>
                      Based on manual ping tests
                    </div>
                  </div>
                  ...
                  <div style={{
                    ...
                    <div style={{
                      width: devices.filter(d => d.status !== 'unknown').length > 0 
                        ? `${Math.round((devices.filter(d => d.status === 'online').length / devices.filter(d => d.status !== 'unknown').length) * 100)}%`
                        : '0%',
                  ...
                  <div style={{
                    marginTop: '16px', color: '#6366f1', fontSize: '14px',
                    fontWeight: '500'
                  }}>
                    {devices.filter(d => d.status === 'online').length} online of {devices.filter(d => d.status !== 'unknown').length} tested devices
                  </div>
                </div>
              </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <span style={{ fontSize: '14px', color: '#1e293b' }}>Online Devices</span>
                    </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <span style={{ fontSize: '14px', color: '#1e293b' }}>Offline Devices</span>
                    </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <span style={{ fontSize: '14px', color: '#1e293b' }}>Not Tested</span>
                    </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <span style={{ fontSize: '14px', color: '#1e293b' }}>Production Systems</span>
                    </div>
                    <span style={{ fontSize: '16px', fontWeight: '600', color: '#1e293b' }}>
                      {devices.filter(d => d.environment === 'production').length}
                    </span>
                  </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <span style={{ fontSize: '14px', color: '#1e293b' }}>Critical Systems</span>
                    </div>
                    <span style={{ fontSize: '16px', fontWeight: '600', color: '#1e293b' }}>
                      {devices.filter(d => d.business_criticality === 'critical').length}
                    </span>
                  </div>

                  {/* Show last manual ping info */}
                  {devices.some(d => d.last_ping) && (
                    <div style={{
                      marginTop: '16px', padding: '12px', background: '#f8fafc',
                      borderRadius: '8px', fontSize: '12px', color: '#64748b'
                    }}>
                      Last manual ping: {
                        devices.filter(d => d.last_ping).length > 0
                          ? new Date(Math.max(...devices.filter(d => d.last_ping).map(d => new Date(d.last_ping).getTime()))).toLocaleTimeString()
                          : 'Never'
                      }
                    </div>
                  )}
              ...
              {/* Recent Activity with Ping Buttons */}
              <div style={{
                background: '#ffffff', borderRadius: '16px', padding: '24px',
                border: '1px solid #e2e8f0', gridColumn: 'span 2'
              }}>
                <h3 style={{ margin: '0 0 20px 0', fontSize: '18px', fontWeight: '600', color: '#1e293b' }}>
                  Device Status & Manual Network Tests
                </h3>
                ...
                {devices.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '40px', color: '#64748b' }}>
                    ...
                    <p style={{ margin: '0', fontSize: '14px' }}>
                      No devices yet. Add devices and test connectivity with ping buttons.
                    </p>
                  </div>
                ) : (
                    ...
                    {devices.slice(0, 5).map((device, i) => (
                      <div key={device.id} style={{
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            ...
                            <span style={{ 
                              ...
                              background: device.status === 'online' ? '#dcfce7' : 
                                         device.status === 'offline' ? '#fef2f2' : '#fef3c7',
                              color: device.status === 'online' ? '#166534' : 
                                     device.status === 'offline' ? '#dc2626' : '#d97706'
                            }}>
                              {device.status === 'unknown' ? 'Not Tested' : device.status}
                            </span>
                            {device.ip_address && (
                          ...
                          <div style={{ fontSize: '12px', color: '#64748b', marginTop: '2px' }}>
                            ...
                            {!device.last_ping && device.ip_address && (
                        ...
                        <div style={{ display: 'flex', gap: '16px', fontSize: '12px', color: '#64748b' }}>
                          {device.response_time > 0 && (
                            <div style={{ textAlign: 'center' }}>
                              <div style={{ 
                                fontWeight: '500', 
                                color: device.response_time < 50 ? '#10b981' : 
                                       device.response_time < 100 ? '#f59e0b' : '#ef4444' 
                              }}>
                                {device.response_time.toFixed(1)}ms
                              </div>
                              <div>Ping</div>
                            </div>
                          )}
                          <div style={{ textAlign: 'center' }}>
                            <div style={{ fontWeight: '500' }}>{device.cpu_usage?.toFixed(0) || 0}%</div>
                            <div>CPU</div>
                          </div>
                          <div style={{ textAlign: 'center' }}>
                            <div style={{ fontWeight: '500' }}>{formatUptime(device.uptime || 0)}</div>
                            <div>Up</div>
                          </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                          {device.ip_address && (
                            <button
                              onClick={() => pingDevice(device.id, device.name, device.ip_address)}
                              disabled={pingingDevices.has(device.id)}
                              style={{
                                background: pingingDevices.has(device.id) ? '#94a3b8' : '#10b981',
                                color: 'white', border: 'none',
                                padding: '6px 12px', borderRadius: '6px', fontSize: '12px',
                                cursor: pingingDevices.has(device.id) ? 'not-allowed' : 'pointer',
                                display: 'flex', alignItems: 'center', gap: '4px'
                              }}
                            >
                              {pingingDevices.has(device.id) ? '🔄' : '🏓'}
                              {pingingDevices.has(device.id) ? 'Pinging...' : 'Ping'}
                            </button>
                          )}
                          <button
                            onClick={() => {
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  <select
                    ...
                    style={{
                      padding: '10px 16px', border: '1px solid #e2e8f0',
                      borderRadius: '8px', fontSize: '14px', minWidth: '120px'
                    }}
                  >
                  ...
                  {/* Clear Filters */}
                  {(searchQuery || filterType || filterEnvironment || filterStatus) && (
                    <button
              ...
              <div style={{ background: '#ffffff', borderRadius: '16px', padding: '24px', border: '1px solid #e2e8f0' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                  <h2 style={{ margin: '0', fontSize: '20px', fontWeight: '600', color: '#1e293b' }}>
                    Device Inventory with Network Testing
                  </h2>
                </div>
                ...
                {filteredDevices.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '60px 20px', color: '#64748b' }}>
                    ...
                    <h3 style={{ margin: '0 0 8px 0', fontSize: '18px', fontWeight: '600', color: '#374151' }}>
                      {devices.length === 0 ? 'No devices registered' : 'No devices found'}
                    </h3>
                    <p style={{ margin: '0', fontSize: '14px' }}>
                      {devices.length === 0 
                        ...
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
                          ...
                          <div style={{ flex: 1 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                              ...
                              {device.response_time > 0 && (
                                <span style={{
                                  ...
                                  background: device.response_time < 50 ? '#dcfce7' : device.response_time < 100 ? '#fef3c7' : '#fef2f2',
                                  color: device.response_time < 50 ? '#166534' : device.response_time < 100 ? '#d97706' : '#dc2626'
                                }}>
                                  {device.response_time.toFixed(1)}ms
                                </span>
                              )}
                            ...
                            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '12px' }}>
                              <span style={{
                                padding: '4px 12px',
                                background: device.environment === 'production' ? '#fecaca' :
                                           device.environment === 'staging' ? '#fef3c7' : '#d1fae5',
                                color: device.environment === 'production' ? '#dc2626' :
                                       device.environment === 'staging' ? '#d97706' : '#059669',
                            ...
                            <div style={{ display: 'flex', gap: '12px', fontSize: '12px', color: '#64748b' }}>
                              <span>CPU: {device.cpu_usage?.toFixed(1) || 0}%</span>
                              <span>RAM: {device.memory_usage?.toFixed(1) || 0}%</span>
                              <span>Up: {formatUptime(device.uptime || 0)}</span>
                            </div>
                          </div>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            {device.ip_address && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation()
                                  pingDevice(device.id, device.name, device.ip_address)
                                }}
                                disabled={pingingDevices.has(device.id)}
                                style={{
                                  background: pingingDevices.has(device.id) ? '#94a3b8' : '#10b981',
                              ...
                              >
                                {pingingDevices.has(device.id) ? '🔄' : '🏓'}
                                {pingingDevices.has(device.id) ? 'Ping...' : 'Test'}
                              </button>
                            )}
                            <button
          ...
          {activeTab === 'security' && (
            <div style={{
              background: '#ffffff', borderRadius: '16px', padding: '60px',
              border: '1px solid #e2e8f0', textAlign: 'center'
            }}>
              <div style={{ fontSize: '64px', marginBottom: '24px', opacity: '0.3' }}>🔒</div>
              <h2 style={{ margin: '0 0 12px 0', fontSize: '24px', fontWeight: '600', color: '#1e293b' }}>
                Security Center
              </h2>
              <p style={{ margin: '0', color: '#64748b', fontSize: '16px' }}>
                Advanced security features coming soon. Monitor device vulnerabilities and security alerts.
              </p>
            </div>
          )}
        </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '14px', color: '#64748b', textTransform: 'capitalize' }}>
                    {selectedDevice.status === 'unknown' ? 'Not Tested' : selectedDevice.status}
                  </span>
                  {selectedDevice.last_ping && (
                    <span style={{ fontSize: '14px', color: '#64748b' }}>
                      • Last ping: {new Date(selectedDevice.last_ping).toLocaleString()}
                    </span>
                  )}
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                {selectedDevice.ip_address && (
                  <button
                    onClick={() => pingDevice(selectedDevice.id, selectedDevice.name, selectedDevice.ip_address)}
                    disabled={pingingDevices.has(selectedDevice.id)}
                    style={{
                      background: pingingDevices.has(selectedDevice.id) ? '#94a3b8' : '#10b981',
                      color: 'white', border: 'none',
                      padding: '8px 16px', borderRadius: '6px', fontSize: '14px',
                      cursor: pingingDevices.has(selectedDevice.id) ? 'not-allowed' : 'pointer'
                    }}
                  >
                    {pingingDevices.has(selectedDevice.id) ? 'Pinging...' : 'Ping Test'}
                  </button>
                )}
                <button
              ...
              <div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', marginBottom: '32px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                      <div>
                        <span style={{ fontSize: '12px', color: '#64748b', textTransform: 'uppercase' }}>Device Type</span>
                        <div style={{ fontSize: '14px', fontWeight: '500', color: '#1e293b' }}>
                          {getDeviceIcon(selectedDevice.device_type)} {
                            deviceTypes.find(t => t.value === selectedDevice.device_type)?.label || selectedDevice.device_type
                          }
                        </div>
                      </div>
                      
                      <div>
                        <span style={{ fontSize: '12px', color: '#64748b', textTransform: 'uppercase' }}>Operating System</span>
                        <div style={{ fontSize: '14px', fontWeight: '500', color: '#1e293b' }}>
                          {selectedDevice.operating_system} {selectedDevice.os_version}
                        </div>
                      </div>
                      
                      <div>
                        <span style={{ fontSize: '12px', color: '#64748b', textTransform: 'uppercase' }}>Network</span>
                        <div style={{ fontSize: '14px', fontWeight: '500', color: '#1e293b' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                      <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                          ...
                          <span style={{ fontSize: '12px', fontWeight: '500', color: getStatusColor(selectedDevice.status) }}>
                            {selectedDevice.status === 'unknown' ? 'NOT TESTED' : selectedDevice.status.toUpperCase()}
                          </span>
                        </div>
                      </div>

                      {selectedDevice.response_time > 0 && (
                        <div>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                            <span style={{ fontSize: '12px', color: '#64748b' }}>RESPONSE TIME</span>
                            <span style={{ fontSize: '12px', fontWeight: '500', color: '#1e293b' }}>
                              {selectedDevice.response_time.toFixed(1)}ms
                            </span>
                          </div>
                        </div>
                      )}
                      ...
                      {selectedDevice.last_ping && (
                        <div>
                          <span style={{ fontSize: '12px', color: '#64748b' }}>LAST PING</span>
                      ...
                      <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                          <span style={{ fontSize: '12px', color: '#64748b' }}>CPU USAGE</span>
                          <span style={{ fontSize: '12px', fontWeight: '500', color: '#1e293b' }}>
                        ...
                        <div style={{ width: '100%', height: '6px', background: '#f1f5f9', borderRadius: '3px' }}>
                          <div style={{
                            ...
                            height: '100%',
                            background: selectedDevice.cpu_usage > 80 ? '#ef4444' : 
                                       selectedDevice.cpu_usage > 60 ? '#f59e0b' : '#10b981',
                            borderRadius: '3px'
                          }}></div>
                        </div>
                      </div>
                      ...
                      <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                          <span style={{ fontSize: '12px', color: '#64748b' }}>MEMORY USAGE</span>
                          <span style={{ fontSize: '12px', fontWeight: '500', color: '#1e293b' }}>
                        ...
                        <div style={{ width: '100%', height: '6px', background: '#f1f5f9', borderRadius: '3px' }}>
                          <div style={{
                            ...
                            height: '100%',
                            background: selectedDevice.memory_usage > 80 ? '#ef4444' : 
                                       selectedDevice.memory_usage > 60 ? '#f59e0b' : '#10b981',
                            borderRadius: '3px'
                          }}></div>
                        </div>
                      </div>
                      ...
                      <div>
                        <span style={{ fontSize: '12px', color: '#64748b' }}>UPTIME</span>
                        <div style={{ fontSize: '14px', fontWeight: '500', color: '#1e293b' }}>
                ...
                {/* Action Buttons */}
                <div style={{ 
                  marginTop: '32px', paddingTop: '24px', borderTop: '1px solid #e2e8f0',
                  display: 'flex', gap: '12px', justifyContent: 'flex-end'
                }}>
                  <button
                    onClick={() => deleteDevice(selectedDevice.id, selectedDevice.name)}
                    style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                ...
                <form onSubmit={updateProfile} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setProfileUpdate({...profileUpdate, full_name: e.target.value})}
                      style={{
                        width: '100%', padding: '10px 12px', border: '1px solid #e2e8f0',
                        borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box'
                      }}
                  ...
                  <button
                    ...
                    disabled={loading}
                    style={{
                      padding: '12px', background: loading ? '#94a3b8' : '#6366f1',
                      color: 'white', border: 'none', borderRadius: '8px',
                      cursor: loading ? 'not-allowed' : 'pointer',
                      fontSize: '14px', fontWeight: '500'
                    }}
              ...
              <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: '24px' }}>
                ...
                <form onSubmit={changePassword} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  <button
                    type="submit"
                    disabled={loading || !passwordChange.current_password || !passwordChange.new_password}
                    style={{
                      padding: '12px', 
                      background: (loading || !passwordChange.current_password || !passwordChange.new_password) 
                        ? '#94a3b8' : '#10b981',
                      color: 'white', border: 'none', borderRadius: '8px',
                      cursor: (loading || !passwordChange.current_password || !passwordChange.new_password) 
                        ? 'not-allowed' : 'pointer',
                      fontSize: '14px', fontWeight: '500'
                    }}
                  >
              ...
              <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: '24px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '14px', color: '#64748b' }}>
                  ...
                  {userProfile.last_login && (
                    <div>
                      <strong>Last login:</strong> {new Date(userProfile.last_login).toLocaleString()}
                    </div>
                  )}
                </div>
              </div>
      ...
      {showAddDevice && (
        <div style={{
          ...
          <div style={{
            ...
            <form onSubmit={addDevice} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              {/* Device Name */}
              <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewDevice({...newDevice, environment: e.target.value})}
                    style={{
                      width: '100%', padding: '12px 16px', border: '1px solid #e2e8f0',
                      borderRadius: '8px', fontSize: '14px', boxSizing: 'border-box'
                    }}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewDevice({...newDevice, business_criticality: e.target.value})}
                    style={{
                      width: '100%', padding: '12px 16px', border: '1px solid #e2e8f0',
                      borderRadius: '8px', fontSize: '14px', boxSizing: 'border-box'
                    }}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setNewDevice({...newDevice, owner_contact: e.target.value})}
                    style={{
                      width: '100%', padding: '12px 16px', border: '1px solid #e2e8f0',
                      borderRadius: '8px', fontSize: '14px', boxSizing: 'border-box'
                    }}
      ...
      {message && (
        <div style={{
          ...
          background: message.includes('✅') || message.includes('🟢') ? '#10b981' : 
                     message.includes('🟡') ? '#f59e0b' : '#ef4444',
          color: 'white', padding: '16px 20px', borderRadius: '8px',
